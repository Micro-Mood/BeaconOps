...
| Broker 地址 | `mqtts://YOUR_BROKER_HOST:8883` | **`components/config/config.h` `BROKER_URI`** | 随主固件同烧;CA 默认用 `certs.c` 内置的 ISRG Root X1(Let's Encrypt) |

**传输层安全**:设备 ↔ `YOUR_BROKER_HOST:8883` 走 TLS 1.2/1.3;TLS 由服务器 nginx `stream` 块终止,代理到本机 `127.0.0.1:1883` 的 Mosquitto(不对公网暴露)。证书复用部署域名的 Let's Encrypt,由 80 端口 acme-challenge 续签。配置位于 `Server/services/beacon/scripts/nginx.stream.conf`,由 `/etc/nginx/nginx.conf` 顶层 `include /opt/server/services/*/scripts/nginx.stream.conf;` 收入。
